...
class LMS:
    ...
    def display_books(self):
        print("------------------------List of Books---------------------")
        print("{:<10} {:^40}".format('Books ID', 'Title [Status]'))
        print("----------------------------------------------------------")
    ...
    def lend_books(self):
        ...
        if books_id in self.books_dict.keys():
            if self.books_dict[books_id]['status'] != 'Available':
                print(
                    f"This book is already issued by {self.books_dict[books_id]['lender_name']} on {self.books_dict[books_id]['lend_date']}")
                # return self.lend_books()
            elif self.books_dict[books_id]['status'] == 'Available':
                your_name = input("Enter Your Name : ")
                self.books_dict[books_id]['lender_name'] = your_name
                self.books_dict[books_id]['lend_date'] = current_date
                self.books_dict[books_id]['status'] = f'Issued by {your_name}'
    ...
    def add_books(self):
        ...
        if new_books == "":
            print(f'Please Enter Valid Book Title')
            return self.add_books()
        # elif len(new_books) > 100:
        #     print("Books title length is too long !!! Title length limit is 100 characters")
        #     return self.add_books()
        else:
...
if __name__ == "__main__":
    try:
        mylms = LMS("list_of_books.txt", "Sandip Salunkhe")
        press_key_list = {1: "Display Books", 2: "Lend Books", 3: "Add Books", 4: "Return Books", 5: "Quit"}
